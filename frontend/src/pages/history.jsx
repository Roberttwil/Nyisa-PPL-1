...
const History = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const notRated = filtered1.filter(item => !submittedRatingIds.includes(item.transaction_id));
        const rated = filtered1.filter(item => submittedRatingIds.includes(item.transaction_id));
    ...
    fetchData();
  }, [submittedRatings]); // Add submittedRatings to dependency array
  ...
  const submitRating = async (restaurant_id, transactionId) => {
    ...
    try {
      ...
      const ratedTransaction = status1.find(item => item.transaction_id === transactionId);
      if (ratedTransaction) {
        setStatus1(prev => prev.filter(item => item.transaction_id !== transactionId));
  ...
  const renderStars = (transactionId, isReadOnly = false) => {
    ...
    return [...Array(5)].map((_, i) => (
      <span
        key={i}
        onClick={() => !isLocked && !isReadOnly && handleRatingClick(transactionId, i + 1)}
        style={{
  ...
  return (
    <div className="flex flex-col min-h-screen max-w-6xl mx-auto px-4 py-8">
      ...
      {loading ? (
        ...
        <>
          {status0.length === 0 && status1.length === 0 && completedTransactions.length === 0 ? (
